...
export default function ProjectsPage() {
  ...
  const recalculateAllStatsMutation = useMutation({
    mutationFn: async () => {
      const projectsData = queryClient.getQueryData(["/api/v1/projects"]) as any[] || [];
      const promises = projectsData.map((project: any) => 
        apiRequest("POST", `/api/v1/projects/${project.id}/recalculate-stats`)
      );
                            ...
                            <div className="text-right min-w-0 space-y-2">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-gray-500 mt-1">
                                  {project.totalSegments > 0 
                                    ? `${Math.round((project.transcribedSegments / project.totalSegments) * 100)}% ${t('transcribed')}`
                                    : project.status === 'ready_for_transcription' 
                                      ? t('readyForTranscription') || 'Ready for transcription'
                                      : t('processing') + "..."
                                  }
                                </p>
                              </div>
                              ...
                              <div>
                                ...
                                <p className="text-xs text-gray-500 mt-1">
                                  {project.totalSegments > 0 
                                    ? `${Math.round((project.translatedSegments / project.totalSegments) * 100)}% ${t('translated')}`
                                    : project.status === 'ready_for_transcription' 
                                      ? t('readyForTranslation') || 'Ready for translation'
                                      : t('processing') + "..."
                                  }
                                </p>
                              </div>
