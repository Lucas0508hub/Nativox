...
export default function ProjectDetailPage() {
  ...
  const { data: languages = [] } = useQuery<any[]>({
    queryKey: ["/api/v1/languages"],
    retry: false,
  });

  const createFolderMutation = useMutation({
    mutationFn: async (data: { name: string; description?: string }) => {
      return await apiRequest("POST", `/api/v1/folders/project/${projectId}`, data);
    },
    onSuccess: () => {
  ...
  const updateFolderMutation = useMutation({
    mutationFn: async (data: { id: number; name: string; description?: string }) => {
      return await apiRequest("PATCH", `/api/v1/folders/${data.id}`, {
        name: data.name,
  ...
  const deleteFolderMutation = useMutation({
    mutationFn: async (folderId: number) => {
      return await apiRequest("DELETE", `/api/v1/folders/${folderId}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [`/api/v1/folders/project/${projectId}`] });
      toast({
  ...
  const updateProjectMutation = useMutation({
    mutationFn: async (data: { name: string }) => {
      return await apiRequest("PATCH", `/api/v1/projects/${projectId}`, data);
    },
    onSuccess: () => {
  ...
  const recalculateStatsMutation = useMutation({
    mutationFn: async () => {
      return await apiRequest("POST", `/api/v1/projects/${projectId}/recalculate-stats`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [`/api/v1/projects/${projectId}`] });
      queryClient.invalidateQueries({ queryKey: ["/api/v1/projects"] });
