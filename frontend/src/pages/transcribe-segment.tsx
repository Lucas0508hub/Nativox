...
export default function TranscribeSegmentPage() {
  ...
  const { data: segment, isLoading: segmentLoading } = useQuery<Segment>({
    queryKey: [`/api/v1/segments/${segmentIdNum}`],
    enabled: !!segmentIdNum,
  });

  const { data: folder } = useQuery<Folder>({
    queryKey: [`/api/v1/folders/${folderIdNum}`],
    enabled: !!folderIdNum,
  });

  const { data: allSegments = [] } = useQuery<Segment[]>({
    queryKey: [`/api/v1/segments/folder/${folderIdNum}`],
    enabled: !!folderIdNum,
  });
  ...
  useEffect(() => {
    ...
    const loadAudio = async () => {
      try {
        const response = await authenticatedFetch(`/api/v1/segments/${segmentIdNum}/audio`);
  ...
  const saveMutation = useMutation({
    mutationFn: async (genreOverride?: string) => {
      return await apiRequest("PATCH", `/api/v1/segments/${segmentIdNum}`, {
        transcription,
        translation,
        is_transcribed: transcription.trim().length > 0,
        is_translated: translation.trim().length > 0,
        genre: genreOverride || segment?.genre,
      });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [`/api/v1/segments/${segmentIdNum}`] });
      queryClient.invalidateQueries({ queryKey: [`/api/v1/segments/folder/${folderIdNum}`] });
      queryClient.invalidateQueries({ queryKey: ["/api/v1/projects"] });
      queryClient.invalidateQueries({ queryKey: [`/api/v1/projects/${projectIdNum}`] });
      toast({
        title: t("success"),
