...
export default function UsersAdminPage() {
  ...
  const deactivateUserMutation = useMutation({
    mutationFn: async (userId: string) => {
      await apiRequest("PATCH", `/api/v1/users/${userId}/deactivate`);
    },
    onSuccess: () => {
  ...
  const resetPasswordMutation = useMutation({
    mutationFn: async ({ userId, newPassword }: { userId: string; newPassword: string }) => {
      await apiRequest("POST", `/api/v1/users/${userId}/reset-password`, { newPassword });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/v1/users"] });
      toast({
  ...
  const handleDeactivateUser = async (user: UserWithLanguages) => {
    const confirmed = await confirm(
      t('confirmDeactivateUser', { userName: user.username }), 
      {
