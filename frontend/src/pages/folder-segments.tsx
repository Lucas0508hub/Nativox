...
export default function FolderSegmentsPage() {
  ...
  const [sortBy, setSortBy] = useState<'segmentNumber' | 'name' | 'duration' | 'date' | 'genre'>('name');
  ...
  const uploadSegmentsMutation = useMutation({
    mutationFn: async (files: File[]) => {
      ...
      return await apiRequest("POST", `/api/v1/folders/${folderIdNum}/upload-segments`, formData);
    },
    onSuccess: () => {
  ...
  const deleteSegmentMutation = useMutation({
    mutationFn: async (segmentId: number) => {
      await apiRequest("DELETE", `/api/v1/segments/${segmentId}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [`/api/v1/segments/folder/${folderIdNum}`] });
      toast({
  ...
  const reorderSegmentsMutation = useMutation({
    mutationFn: async (segmentIds: number[]) => {
      await apiRequest("PATCH", `/api/v1/folders/${folderIdNum}/reorder-segments`, {
        segmentIds
      });
    },
    onSuccess: () => {
  ...
  const handleExportCSV = async () => {
    try {
      const response = await apiRequest('GET', `/api/v1/folders/${folderId}/export`);
